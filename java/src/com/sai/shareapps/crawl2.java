...
public class crawl2 {
	...
	public static void main(String[] args) {
		...
		final String FILE_HEADER = "symbol,price,d2e,eps,pe,roce5y,roce1y,avgdiv3y,avgDivPay,high52,low52,profit5y,profit3y,profit1y,opm,sales5y,sales3y,sales1y,div,bv";
		String line = "";
		String Record = "";
		try {
			...
			while ((symbol = fileReader.readLine()) != null) {
				...
				//--
			//	driver.findElement(By.xpath("//*[@id=\"content\"]/div/nav/div/div[1]/div/div/div/div/input")).sendKeys(symbol.trim());
			//	driver.findElement(By.xpath("//*[@id=\"content\"]/div/nav/div/div[1]/div/div/div/span/button")).;  
			 ...
			 try
			 {
				// get the actual value of the title
				 List<WebElement> price = driver.findElements(By
							.xpath("//*[@id=\"content\"]/div/div/div/section[1]/div[1]/h4[2]/b"));
				 List<WebElement> debt2equity = driver.findElements(By
							.xpath("//*[@id=\"content\"]/div/div/div/section[1]/div[2]/h4[1]"));
				 List<WebElement> pe = driver.findElements(By
							.xpath("//*[@id=\"content\"]/div/div/div/section[1]/div[1]/h4[4]/b"));
				 List<WebElement> roce5y = driver.findElements(By
							.xpath("//*[@id=\"content\"]/div/div/div/section[1]/div[2]/h4[3]"));
				 List<WebElement> roce1y = driver.findElements(By
							.xpath("//*[@id=\"content\"]/div/div/div/section[1]/div[2]/h4[4]"));
				 List<WebElement> eps = driver.findElements(By
							.xpath("//*[@id=\"content\"]/div/div/div/section[1]/div[2]/h4[2]"));
				 List<WebElement> avgDiv3y = driver.findElements(By
							.xpath("//*[@id=\"content\"]/div/div/div/section[1]/div[2]/h4[5]"));
...
*/
				//System.out.println(price.get(0)+","+debt2equity.get(0));
				Record = symbol
						+ ","
						+ ((price.size() > 0 ) ? price.get(0).getText().replace('₹', ' ').trim().replaceAll(",",""):"-1")
						+ ","
						+ ((debt2equity.size() > 0 ) ? debt2equity.get(0).getText().split(" ")[3]:"-1")
						+ ","
						+ ((eps.size() > 0 ) ? eps.get(0).getText().split(" ")[2].replace('₹', ' ').trim():"-1")
						+ ","
						+ ((pe.size() > 0 ) ? pe.get(0).getText().replaceAll(",",""):"-1")
						+ ","
						+ ((roce5y.size() > 0 ) ? roce5y.get(0).getText().split(" ")[6].replaceAll(",", "").replace('%', ' ').trim():"-1")
						+","
						+ ((roce1y.size() > 0 ) ? roce1y.get(0).getText().split(" ")[6].replaceAll(",", "").replace('%', ' ').trim():"-1")
						+","
						+ ((avgDiv3y.size() > 0 ) ? avgDiv3y.get(0).getText().split(" ")[4].replaceAll(",", "").replace('%', ' ').trim():"-1");
					/*	+","
